...
export type AslyIncentiveDocument = AslyIncentive & Document;
...
export class RewardStreams {
  ...
  @Prop({ type: MongooseSchema.Types.ObjectId, ref: UserType.name, required: false })
  actionFrom: string;
}
export class Segmentations {
  @Prop({ type: MongooseSchema.Types.ObjectId, ref: Segment.name, required: false })
  segment: string;
...
export class AslyIncentive {
  ...
  @Prop({ type: MongooseSchema.Types.ObjectId, ref: PriceList.name, required: true })
  priceList: string;
...
const AslyIncentiveSchema = SchemaFactory.createForClass(AslyIncentive);
AslyIncentiveSchema.index({ vertical: 1, supplier: 1, userType: 1, productType: 1 }); //compound index

export { AslyIncentiveSchema };
